...
class DataFlowSanitizer : public ModulePass {
  ...
  FunctionType *DFSanTestFnTy;
  FunctionType *DFSanHookDebugFnTy;
  FunctionCallee SetSourceFn;
  FunctionCallee DFSanVaArgHook1Fn;
  FunctionCallee DFSanHook1Fn;
  FunctionCallee DFSanHook2Fn;
...
bool DataFlowSanitizer::doInitialization(Module &M) {
  ...
  Type *SetSource[1] = {Int32Ty};
  SetSourceFnTy = 
      FunctionType::get(Type::getVoidTy(*Ctx), SetSource, /*isVarArg=*/false);
  ...
  Type *DFSanHook1Arg[2] = {Int32Ty, ShadowTy}; 
  DFSanHook1FnTy = 
      FunctionType::get(Type::getVoidTy(*Ctx), DFSanHook1Arg, false);
  Type *DFSanHook2Arg[3] = {Int32Ty, ShadowTy, ShadowTy}; 
  DFSanHook2FnTy = 
      FunctionType::get(Type::getVoidTy(*Ctx), DFSanHook2Arg, false);
...
void DataFlowSanitizer::initializeRuntimeFunctions(Module &M) {
  ...
  DFSanHook1Fn = Mod->getOrInsertFunction("__dfsan_hook1", DFSanHook1FnTy);
  DFSanHook2Fn = Mod->getOrInsertFunction("__dfsan_hook2", DFSanHook2FnTy);
  ...
  DFSanTestFn = Mod->getOrInsertFunction("__dfsan_test", DFSanTestFnTy);
...
bool DataFlowSanitizer::runOnModule(Module &M) {
  ...
  for (Function &i : M) {
    if (!i.isIntrinsic() &&
        ...
        &i != DFSanHook1Fn.getCallee()->stripPointerCasts() &&
        &i != DFSanHook2Fn.getCallee()->stripPointerCasts() &&
        ...
        &i != DFSanTestFn.getCallee()->stripPointerCasts() &&
...
void DFSanVisitor::visitBinaryOperator(BinaryOperator &BO) {
  // TODO: add integer overflow propagation here!
  // BI is the place that insert the taint propagation part
...
void DFSanVisitor::visitCallBase(CallBase &CB) {
  ...
  if (ClHookInst && F && !F->isIntrinsic() && Caller->getName().substr(0, 5) != "dfsw$" && DFSF.DFS.isHooked(F)) {
    ...
    switch(DFSF.DFS.getHookType(F)) {
      case DataFlowSanitizer::DFSH_HOOK1:
        IRB.CreateCall(DFSF.DFS.DFSanHook1Fn, ArgArray);
        break;
...
static RegisterPass<DataFlowSanitizer> X("dfsan", "DFSanPass", false, false);

static void registerDFSan(const PassManagerBuilder &,
...
static RegisterStandardPasses
    RegisterDFSan(PassManagerBuilder::EP_OptimizerLast,
                   registerDFSan);

static RegisterStandardPasses
    RegisterDFSan0(PassManagerBuilder::EP_EnabledOnOptLevel0,
                   registerDFSan);
